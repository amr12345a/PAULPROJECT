param(
	[string]$InstallDir = "$env:ProgramData\IBKRTradeExecutor",
	[string]$TaskName = 'IBKRTradeExecutor',
...
function Get-SupportedPythonLauncher {
	...
	throw 'Python 3.12 or 3.13 is required. Python 3.14 may force a source build of pydantic-core, which needs Rust/Cargo.'
}
...
if ($OpenFirewall) {
	$ruleName = "IBKRTradeExecutor-$Port"
